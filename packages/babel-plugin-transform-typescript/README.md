...
* Does not support [`export =`][exin] and [`import =`][exin], because those cannot be compile to ES.next.

## Workarounds

* `namespace`: Move to using [file exports][fm], or migrate to using the `module { }` syntax instead.
* `const enum`: Remove the `const`, which makes it available at runtime.
* `export =` \ `import =`: Convert to using `export default` and `export const`, and `import x, {y} from "z"`.
